...
export const TransferModal: React.FC<TransferModalProps> = ({
  ...
  const [isCashless, setIsCashless] = useState(false); // Добавляем состояние для безналичного платежа
  ...
  // Проверяем, является ли источник сотрудником (row === 2)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await transferFunds(
        ...
        isEmployeeSource ? {
          isSalary,
          isCashless // Передаем флаг безналичного платежа
        } : undefined
      );
      onClose();
    } catch (error) {
